...
"""
    ...
    try:
        ...
        pass
    backup_paths = [
        "/storage/emulated/0/MyHits/baymaxhits.txt",              
    ]
...
YELLOW = "\033[93m"
RESET = "\033[0m"
print(f"\n{BOLD}{CYAN}Select a Year or Custom Range:{RESET}\n")
for k in range(1, 10):
    print(f"{YELLOW}{k}{RESET} - {2010 + k}")
print(f"{YELLOW}Custom{RESET} - e.g. 4-6 to combine ranges (2014–2016)")
year_input = safe_int_input(f"\n{BOLD}Enter your choice (1–9 or 4-6): {RESET}", "5")
if "-" in year_input:
    try:
        start_key, end_key = map(int, year_input.split("-"))
        if start_key in ranges and end_key in ranges and start_key <= end_key:
            start, _ = ranges[start_key]
            _, end = ranges[end_key]
            selected_range = (start, end)
            print(f"\n💦 You selected custom range: {CYAN}{2010 + start_key}-{2010 + end_key}{RESET}")
        else:
            raise ValueError
    except:
        print(f"\n❌ Invalid input, defaulting to {CYAN}2015{RESET}")
        selected_range = ranges[5]
else:
    try:
        choice = int(year_input)
        if choice in ranges:
            selected_range = ranges[choice]
            print(f"\n💦 You selected: {CYAN}{2010 + choice}{RESET}")
        else:
            raise ValueError
    except:
        print(f"\n❌ Invalid input, defaulting to {CYAN}2015{RESET}")
...
def gg(min_followers, min_posts, user_id_func, color="\033[96m"):
    while True:
        try:
            ...
            pass
minimum_followers = styled_input("👥 Enter minimum followers needed:", 0, "\033[96m")
minimum_posts = styled_input("📝 Enter minimum number of posts needed:", 0, "\033[92m")
for _ in range(100):Thread(target=gg,args=(minimum_followers,minimum_posts,generate_user_id)).start()
